...
PluginEditor::PluginEditor(PluginProcessor &p)
  ...
  // Load background image (V2 path)
  juce::File bgFile("f:\\S612VSTiV2\\AKAK_S6I2_background.jpg");
  if (!bgFile.existsAsFile())
    bgFile = juce::File("f:\\S612VSTiV2\\backpanel_clean2.png");
  if (!bgFile.existsAsFile())
    bgFile = juce::File("f:\\S612VSTi\\AKAK_S6I2_background.jpg");
  ...
  // ─── FILTER PANEL ─────────────────────────────────────────────────────────
  m_filterPanel = std::make_unique<UI::FilterPanel>(apvts);
  ...
  // Note: filter knobs are owned by m_filterPanel and have their own MIDI-learn
  ...
  startTimerHz(30);
  // ─── FINAL LAYOUT ─────────────────────────────────────────────────────────
  setSize(1000, 250); // Ensure this is called last so all components are
...
void PluginEditor::resized() {
  ...
  // ─── FAR RIGHT SECTION: SMALL KNOBS ROW
  // ───────────────────────────────────── 6 knobs: Speed, Depth, Delay, Tune,
  // Decay, Level
  const int sx = 672, sy = 34, sw = 36, sh = 58, gap = 48;

  m_lfoSpeedKnob.setBounds(sx + gap * 0, sy, sw, sh);
  m_lfoDepthKnob.setBounds(sx + gap * 1, sy, sw, sh);
  m_lfoDelayKnob.setBounds(sx + gap * 2, sy, sw, sh);
  m_tuneKnob.setBounds(sx + gap * 3, sy, sw, sh);
  m_decayKnob.setBounds(sx + gap * 4, sy, sw, sh);
  m_levelKnob.setBounds(sx + gap * 5, sy, sw, sh);

  // ─── FILTER PANEL: Centered below the knobs
  // ────────────────────────────────── Spanning from the first knob's left to
  // the last knob's right
  if (m_filterPanel) {
    const int panelW = (gap * 5) + sw + 10;
    m_filterPanel->setBounds(sx - 5, 105, panelW, 130);
  }
}
