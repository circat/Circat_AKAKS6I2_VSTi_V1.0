...
void S612AnalogFilter::updateCoefficients() {
  ...
  // Ensure cutoff is within stable range
  float safeCutoff =
      juce::jlimit(20.0f, (float)m_sampleRate * 0.49f, m_currentCutoffHz);

  auto coeffs1 =
      juce::IIRCoefficients::makeLowPass(m_sampleRate, safeCutoff, q1);
  auto coeffs2 =
      juce::IIRCoefficients::makeLowPass(m_sampleRate, safeCutoff, q2);
