...
void S612Engine::prepareToPlay(double sampleRate,
  ...
  m_writePosition = 0;
  m_inputLevel = 0.0f;
  m_lastInputLevel = 0.0f;
  m_warmUpCounter = 30; // ~150ms at 44.1k/256
...
void S612Engine::processBlock(juce::AudioBuffer<float> &buffer,
  ...
  int filterTypeVal = (int)getParam(filterType, 0.0f);
  float filterResVal = getParam(filterResonance, 0.0f);
  float filterSwpVal = getParam(filterSweep, 0.0f);
      ...
      if (data) {
        for (int i = 0; i < numSamples; ++i) {
          ...
          if (std::isfinite(peak))
            currentBlockMax =
                juce::jmax(currentBlockMax, juce::jmin(2.0f, peak));
        }
      }
    }
  }
  ...
  if (m_voiceManager) {
    ...
    m_voiceManager->setFilterType(filterTypeVal);
    m_voiceManager->setFilterResonance(filterResVal);
    m_voiceManager->setFilterSweep(filterSwpVal);
...
void S612Engine::loadSample(const juce::File &file, double targetRate) {
  ...
  if (auto reader = std::unique_ptr<juce::AudioFormatReader>(
    ...
    loadSampleFromReader(std::move(reader), targetRate);
  }
}

void S612Engine::loadSampleFromMemory(const void *data, size_t size,
                                      double targetRate) {
  juce::AudioFormatManager formatManager;
  formatManager.registerBasicFormats();
...
void S612Engine::loadSampleFromReader(
    std::unique_ptr<juce::AudioFormatReader> reader, double targetRate) {
  if (!reader)
    return;
