...
void S612Voice::startNote(int midiNote, float velocity) {
  ...
  // Configure filter: convert normalised cutoff to Hz (200-18000) then set
  // type/res
  {
    const float minHz = 200.0f, maxHz = 18000.0f;
    float cutoffHz = minHz * std::pow(maxHz / minHz, m_filterCutoff);
    m_filter.updateCoefficients(cutoffHz, m_filterResonance, m_filterType);
    m_filter.setSEMSweep(m_filterSweep);
  }
  ...
  float rateRatio =
      (m_sampleRate > 0) ? (float)(m_originalSampleRate / m_sampleRate) : 1.0f;
...
void S612Voice::renderNextBlock(juce::AudioBuffer<float> &buffer,
  ...
  // Update filter coefficients once per block (not per sample, for performance)
  {
    const float minHz = 200.0f, maxHz = 18000.0f;
    float cutoffHz = minHz * std::pow(maxHz / minHz, m_filterCutoff);
    m_filter.updateCoefficients(cutoffHz, m_filterResonance, m_filterType);
  ...
  for (int i = startSample; i < startSample + numSamples; ++i) {
    ...
    // Apply 4th-order Analog Filter (CircuitBentFilter ZDF)
    float out = m_filter.processSample(envSample);
